...
typedef struct {
    ...
    int *y;
    R_xlen_t *idx;
    R_xlen_t n;      /* number of elements; slot 0 unused */
    R_xlen_t cap;    /* allocated slots, excluding slot 0 */
} Heap;
...
/* R_alloc memory cannot be realloc'ed, so growth copies into a fresh
 * block. Both are freed when .Call returns. */
